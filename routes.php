...
$app->group('/api', function () use ($app) {
    ...
    //mansion miniprojact
        ...
        $app->get('/mansion', function ($request, $response, $args) {
            $sth = $this->db->prepare("SELECT * FROM mansion ORDER BY mansion");
            $sth->execute();
            $data = $sth->fetchAll();
    ...
    $app->get('/mansion/search/[{query}]', function ($request, $response, $args) {
        $sth = $this->db->prepare("SELECT * FROM mansion WHERE UPPER(ms_name) LIKE :query ORDER BY mansion_id");
        $query = "%".$args['query']."%";
        $sth->bindParam("query", $query);
        $sth->execute();
        $data = $sth->fetchAll();
    ...
    //dormitory miniprojact
        ...
        // Retrieve Dm all record use fetchAll()     
